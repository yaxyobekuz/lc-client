...
const ExpenseTypeEditModal = ({ expenseType, close, isLoading, setIsLoading }) => {
  const [name, setName] = useState(expenseType?.name || "");
  const { mutate } = useExpenseTypeUpdateMutation({
    onSuccess: () => {
      setIsLoading(false);
      close?.();
    },
  ...
  const handleSubmit = (e) => {
    ...
    if (!name.trim()) return;
    setIsLoading(true);
    mutate({ id: expenseType._id, body: { name: name.trim() } });
  };
